...
#define DEFINE_THREE_CONSTRUCTORS(class_name, pointer_to_another_type_object, \
                                  pointer_to_same_type_object, node_number)   \
...
class_name::class_name(OperatorTypeClass* op, std::string node_name) {        \
  ...
  \
class_name::class_name(OperatorTypeClass* op1, OperatorTypeClass* op2,        \
...
DEFINE_THREE_CONSTRUCTORS(Operators, pointer_to_operator, pointer_to_operators,
                          _operators_number);

DEFINE_THREE_CONSTRUCTORS(DefinitionBlocksClass,
                          pointer_to_def_block_with_brace_code,
                          pointer_to_def_blocks,
                          _definition_blocks_class_number);

DEFINE_THREE_CONSTRUCTORS(DefinitionBlockClass, pointer_to_token,
                          pointer_to_def_block, _definition_block_class_number);

OperatorClass::OperatorClass(TokenTypeClass* variable,
                             OperatorTypeClass* definition_block,
                             std::string node_name)
    : definition_block_pointer(definition_block), variable_pointer(variable) {
...
DefinitionBlockWithBraceCode::DefinitionBlockWithBraceCode(
    ...
    : brace_code_pointer(brace_code), block_pointer(block) {
