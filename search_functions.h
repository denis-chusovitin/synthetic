...
/// У нас узлы не одинаковые, поэтому для каждого типа узла сделаем свою
/// функцию.
void searchOperator(OperatorTypeClass* _operator);
void searchOperators(OperatorTypeClass* _operators);
/// Их можно как одну использовать разницы нет, а хотя нет, есть. пока оставим
/// так чтобы каждая была отдельно
