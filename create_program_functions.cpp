...
void calculateVariants() {
  ...
  for (int i = 0; i < generate_functions_data.node_struct_vector.size(); ++i) {
    count_of_variants.push_back(
        generate_functions_data.node_struct_vector[i]->_rule.size());
  }
  int _sum = 0;
  long long _mult = 1;
...
void printMapOfVisitedRules() {
  for (auto i = map_of_visited_rules.begin(); i != map_of_visited_rules.end();
       ++i) {
...
void setupMapOfVisitedRules() {
  for (int i = 0; i < generate_functions_data.node_struct_vector.size(); ++i) {
    ...
    std::vector<bool> temp_mass_of_visited_rules(temp_node->_rule.size(),
                                                 false);
...
void prinFirstPath() {
  ...
  for (int i = 0; i < first_path.size(); ++i) {
    printf("%s: %s\n", first_path[i]._node->_name.c_str(),
           first_path[i]._rule->_name.c_str());
  }
};
// PathTrougthTheGraph *first_path = new
// PathTrougthTheGraph[generate_functions_data.node_struct_vector.size()];
...
RuleStruct* findRulesWithoutClosure(NodeStruct* _node) {
  ...
  for (int i = 0; i < _node->_rule.size(); i++) {
    if ((!isThisRuleHasClosure(_node->_rule[i])) &&
        (_node->_rule[i]->_variable.size() > 0)) {
...
void passWithBreakConnectionNode(NodeStruct* _node) {
  ...
  // Точнее нам нужно проверить не указывает ли оно на узелв котором мы уже
  // были.
...
/////////////////////   ФУНКЦИИ ОБЕСПЕЧИВАЮЩИЕ ГЕНЕРАЦИЮ БОЛЬШОГО КОЛИЧЕСТВА
///ПРОГРАММ
RuleStruct* generateRuleFromMapOfVisitedNodes(NodeStruct* _node) {
  RuleStruct* temp_rule;
  std::vector<RuleStruct*> appropriate_rules;
...
// Функция записывает пройденное правило а если узла еще не было в карте запишет
// его в карту и пометит правило
void writeChoosenRuleIntoMapOfVisitedNodes(NodeStruct* _node,
                                           RuleStruct* _rule) {
  ...
  for (int i = 0; i < first_path.size(); ++i) {
    if (_node == first_path[i]._node) {
      ...
      if (i == first_path.size() - position_in_path) {
        ...
        // А не по пути
        if (isAllRulesVisitedInThisNode(_node)) /* если в карте пути по этому
                                                   знаку все правила посещены */
        {
          position_in_path++;
        }
...
void passByRefrencesNode(NodeStruct* _node) {
  ...
  // т.е. сравнить _node с узлами в пути и если мы здесь были пререходим к
  // проходу по уже пройденному пути
  if (isNodeWasInFirstPath(_node)) {
    ...
    // Иначе просто идем по ссылкам выбирая случайные правила и записывая их в
    // карту посещенных
    // Тепеь сначала определим, какое правило будем использовать
    RuleStruct* temp_rule = generateRuleFromMapOfVisitedNodes(_node);
    // Тпереь записали в карту выбранное правило
    writeChoosenRuleIntoMapOfVisitedNodes(_node, temp_rule);
    /// ?????????????????????????/ Тут надо подумать как быть если еще ни разу
    /// не были в этом месте
    passByRefrencesRule(temp_rule);
  }
};
void passByRefrencesRule(RuleStruct* _rule) {
...
void findRefrenceToSpecificNode_Node(NodeStruct* _node,
                                     PathTrougthTheGraph specific_point);
void findRefrenceToSpecificNode_Rule(RuleStruct* _rule,
                                     PathTrougthTheGraph specific_point);
void findRefrenceToSpecificNode_Variable(VariableStruct* _variable,
                                         PathTrougthTheGraph specific_point);
  ...
  for (int i = 0; i < first_path.size(); ++i) {
    if (_node == first_path[i]._node) {
      if (_node == specific_point._node) {
        ...
        writeChoosenRuleIntoMapOfVisitedNodes(specific_point._node,
                                              specific_point._rule);
        passByRefrencesRule(specific_point._rule);
        break;
      } else {
        writeChoosenRuleIntoMapOfVisitedNodes(first_path[i]._node,
                                              first_path[i]._rule);
...
// Нужна функция "Вернуть Мне Узел и правило которые ссылаеются на имя искомого
// узла
PathTrougthTheGraph returnNodeAndRuleRefrenToSearchedNode(
  ...
  for (int pos_of_node = 0; pos_of_node < first_path.size(); ++pos_of_node) {
    ...
    for (int pos_of_rule = 0;
         pos_of_rule < first_path[pos_of_node]._node->_rule.size();
         ++pos_of_rule) {
      // В этом цикле мы будем смотреть как у нас дела с ссылкой на искомый узел
      // Но вот блин у нас эти ссылки хранятся в векторе переменных для каждого
      // правила
      // Поэтому третьим циклом надо каждую из них проглядеть на предмет ссылки
      // на узлы
      ...
      for (int pos_of_variable = 0;
           pos_of_variable < temp_rule->_variable.size(); ++pos_of_variable) {
        ...
        if (temp_rule->_variable[pos_of_variable]->_node != NULL) {
          ...
          if (searched_node.compare(
                  temp_rule->_variable[pos_of_variable]->_node->_name) == 0) {
            // А что мы собственно теперь должны сделать?
            if (isNodeWasInFirstPath(
                    temp_rule->_variable[pos_of_variable]->_node)) {
              // Немного по другому сделаем Мы вернем узел который хотим
              // посетить, его имя.
              // и вернем правио сгенерированное из карты
              temp_path_point._node =
                  temp_rule->_variable[pos_of_variable]->_node;
              temp_path_point._rule = generateRuleFromMapOfVisitedNodes(
                  temp_rule->_variable[pos_of_variable]->_node);
            } else {
              // Наверное надо записать в нашу структуру узел
              // first_path[pos_of_node]._node
...
bool isMapOfVisitedRulesCompleted() {
  ...
  for (auto i = map_of_visited_rules.begin(); i != map_of_visited_rules.end();
    ...
    for (int pos_in_vector = 0; pos_in_vector < i->second.size();
         ++pos_in_vector) {
...
void generateProgramsWithPassedPath() {
  ...
  while (!isMapOfVisitedRulesCompleted()) {
    ...
    printf("%s\n", searched_node.c_str());
    PathTrougthTheGraph specific_point =
        returnNodeAndRuleRefrenToSearchedNode(searched_node);
    printf("%s: %s \n", specific_point._node->_name.c_str(),
           specific_point._rule->_name.c_str());
...
//// Нужна функция проверки пустого правила. Первое правило проверяем пустое оно
///или нет.
bool isRuleEmpty(RuleStruct* _rule) {
...
bool isNodeWithClosure(
    NodeStruct* _node) {  /// Впоследствии надо будет переделать под
  for (int i = 0; i < _node->_rule.size(); ++i) {
