...
void printAllDataFromNewStruct() {
  for (auto i = print_functions_data.node_struct_vector.begin();
       i != print_functions_data.node_struct_vector.end(); ++i) {
    printNodeData(*i);
  }
  // теперь надо вывести все связи
  /*
    for (auto i=print_functions_data.var_struct_vector.begin(); i !=
    print_functions_data.var_struct_vector.end(); ++i){
      if ((*i)->_node != NULL) {
        printf("%s -> %s;\n",(*i)->_variable_uniq_name.c_str(),
    (*i)->_node->_node_uniq_name.c_str());
      }
    }
  */
  for (auto i = print_functions_data.node_struct_vector.begin();
