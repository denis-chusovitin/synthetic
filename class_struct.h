...
class OperatorClass : public OperatorTypeClass {
 public:
  ...
  OperatorClass(TokenTypeClass* Variable, OperatorTypeClass* definition_block,
                std::string node_name);
  //~OperatorClass();
};
...
class DefinitionBlockWithBraceCode : public OperatorTypeClass {
 public:
  ...
  OperatorTypeClass* block_pointer;  // Ссылается на все перечисленные, кроме
                                     // наверное Brace_code_token
  TokenTypeClass* brace_code_pointer;  // песть отдельно ссылается на код
  // std::string _node_name;
  DefinitionBlockWithBraceCode(OperatorTypeClass* block,
                               TokenTypeClass* Brace_code,
                               std::string node_name);
};
