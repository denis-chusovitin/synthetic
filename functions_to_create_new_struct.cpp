...
std::string returnLableOfOperator(OperatorTypeClass* _operator) {
  ...
  // printf("%s\n","try to go work with Lable of Operator" );
  if (operator_pointer == NULL) {
    printf("%s\n", "mistake");
    return "";
  }
  if (operator_pointer->variable_pointer) {
    // printf("%s\n","fail" );
    Variable* _variable_pointer =
        dynamic_cast<Variable*>(operator_pointer->variable_pointer);
    if (_variable_pointer) {
      std::string temp_str = _variable_pointer->returnName();
      // printf("%s\n", temp_str.c_str() );
      return temp_str;  // Нужно вернуть Лэйбл оператора либо его уникальное имя
                        // не важно
    }
  }
  // printf("%s\n","fail" );
...
void goIntoDefBlockWBCAndFindVariables(OperatorTypeClass* block_pointer,
                                       RuleStruct* temp_rule) {
  ///приведем к удобному типу
  DefinitionBlockClass* def_block =
      dynamic_cast<DefinitionBlockClass*>(block_pointer);
  ...
  if (def_block->pointer_to_def_block != NULL) {
    ...
    goIntoDefBlockWBCAndFindVariables(def_block->pointer_to_def_block,
                                      temp_rule);
  }
  if (def_block->pointer_to_token) {
    TokenClass* token_block =
        dynamic_cast<TokenClass*>(def_block->pointer_to_token);
    ...
    if (token_block->_token) {
      ...
      if (symbol_pointer) {
        ...
        Variable* variable_pointer =
            dynamic_cast<Variable*>(token_block->_token);
        if (variable_pointer != NULL) {
          ...
          if (variable_pointer->def_operator != NULL) {
            ...
            OperatorClass* operator_pointer =
                dynamic_cast<OperatorClass*>(variable_pointer->def_operator);
            temp_variable->_node_name =
                returnLableOfOperator(variable_pointer->def_operator);
          }
          temp_rule->_variable.push_back(temp_variable);
          create_functions_data.var_struct_vector.push_back(temp_variable);
        }
...
void goIntoDefinitionBlocksAndFindDefBlocksWithBraceCode(
    OperatorTypeClass* def_blocks_pointer, NodeStruct* temp_node) {
  DefinitionBlocksClass* def_blocks =
      dynamic_cast<DefinitionBlocksClass*>(def_blocks_pointer);
  if (def_blocks->pointer_to_def_blocks != NULL) {
    goIntoDefinitionBlocksAndFindDefBlocksWithBraceCode(
        def_blocks->pointer_to_def_blocks, temp_node);
  }
  if (def_blocks->pointer_to_def_block_with_brace_code != NULL) {
    // Нашли и теперь записываем к нам в вектор
    DefinitionBlockWithBraceCode* _def_block_w_end =
        dynamic_cast<DefinitionBlockWithBraceCode*>(
            def_blocks->pointer_to_def_block_with_brace_code);
    ...
    // теперь аналогично надо найти все переменные на которые ссылается
    // DefBlockWBC
    if (_def_block_w_end->block_pointer != NULL) {
      goIntoDefBlockWBCAndFindVariables(_def_block_w_end->block_pointer,
                                        temp_rule);
    }
  }
};
...
void createNodesFromMap() {
  for (auto j = create_functions_data.var_map.begin();
       j != create_functions_data.var_map.end(); ++j) {
    NodeStruct* temp_node =
        new NodeStruct(j->second->_node_name, j->first->returnName());
    ...
    goIntoDefinitionBlocksAndFindDefBlocksWithBraceCode(
        searched_operator->definition_block_pointer, temp_node);
    create_functions_data.node_struct_vector.push_back(temp_node);
  }
}

void generateAdditionalConnectionsInNewStructure() {
  for (auto i = create_functions_data.var_struct_vector.begin();
       i != create_functions_data.var_struct_vector.end(); ++i) {
    /// Пройдемся по всем векторам и проверим как
    /// проверим какие имена переменных совпадают с именами переменных в карте и
    /// установим соответствующие связи
    for (auto j = create_functions_data.node_struct_vector.begin();
         j != create_functions_data.node_struct_vector.end(); ++j) {
      // Дальше уже будем проверять и создавать сответствующие связи
      if ((*i)->_node_name == (*j)->_lable_data) {
        // std::cout << "connection created"<<  (*i)->_node_name << " "<<
        // (*j)->_name << std::endl;
        (*i)->_node = (*j);
      }
    }
